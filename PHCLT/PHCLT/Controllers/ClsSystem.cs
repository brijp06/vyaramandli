        ...
        public LoginResponse VerifyUser(string UserName, string Password)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(ConfigurationManager.ConnectionStrings["ConnectionString"].ToString()))
                {
                    ...
                    string phoneUserQuery = "SELECT CustomerId, CustomerName as UserName FROM CustomerMaster WHERE MobNo = @UserName AND YEAR(BirthDate) = @BirthYear";
                    ...
                    {
                        // User not found in UserMaster, check in CustomerMaster
                        using (SqlCommand phoneCommand = new SqlCommand(phoneUserQuery, connection))
                        {
                            phoneCommand.Parameters.AddWithValue("@UserName", userName);
                            phoneCommand.Parameters.AddWithValue("@BirthYear", Password);

                            using (SqlDataReader reader = phoneCommand.ExecuteReader())
                            {
                                if (reader.HasRows)
                                {
                                    // Read values for User
                                    reader.Read();
                                    loginResponse.UserType = "User";
                                    loginResponse.Username = reader["UserName"].ToString();
                                    loginResponse.CustomerId = Convert.ToInt32(reader["CustomerId"]);
                                }
                            }
                        }
                    }
